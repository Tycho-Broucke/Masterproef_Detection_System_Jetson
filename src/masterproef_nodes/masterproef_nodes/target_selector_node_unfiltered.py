...
class TargetSelectorNodeUnfiltered(Node):
    """
    ...
    """
    def __init__(self):
        super().__init__('target_selector_node_unfiltered')
    ...
    def coordinates_callback(self, data):
        ...
        - Transform to SLAM coordinates
        - Store as latest target
        - Attempt to send or cancel goal
        """
        ...
        _, closest_person, fallback_coord = selector.get_shortest_distance(coordinates, 'R', 'P', None)

        # Determine the latest target coordinates
        if closest_person:
            x, y = self.apply_homography(closest_person['x'], closest_person['y'])
            self.latest_target = (x, y)
        elif fallback_coord:
            x, y = self.apply_homography(fallback_coord[0], fallback_coord[1])
            self.latest_target = (x, y)
        else:
            self.latest_target = None
...
def main(args=None):
    ...
    node = TargetSelectorNodeUnfiltered()
    try:
        rclpy.spin(node)
